...
/*
 * create flow to mirror (sampling with ratio=1) matched packet
 * the corresponding testpmd commands:
 * testpmd> set sample_actions 0 mark id 0xbefeb / queue index 2 / end
 * testpmd> flow create 0 ingress group 0 pattern eth / ipv4 / udp /
 *          gtp teid is 1234 msg_type is 255 / ipv4 src is 13.10.11.11 /
 *          tcp / end actions jump group 1 / end
 * testpmd> flow create 0 ingress group 1 pattern eth / ipv4 / udp /
 *          gtp teid is 1234 msg_type is 255 / ipv4 src is 13.10.11.11 /
 *          tcp / end actions sample ratio 1 index 0 / mark id 0xbef /
 *          queue index 1 / end
 */
struct rte_flow *
		...
		uint16_t fwd2queue)
{
	...
	struct rte_flow_item_ipv4 ipv4_inner = {
			.hdr = {
				.src_addr = rte_cpu_to_be_32(0x0D0A0B0B),
				/* Match on 13.10.11.11 src address */
				.next_proto_id = IPPROTO_TCP }};
	struct rte_flow_item_ipv4 ipv4_mask = {
			.hdr = {
				.src_addr = RTE_BE32(0xffffffff)}};
	...
	flow = rte_flow_create(port_id, &attr, pattern, root_actions, &error);
	if (!flow) {
		printf("Can't create jump flow on port: %u, group: %d, error: %s\n",
				port_id, attr.group, error.message);
		return flow;
	}
	struct rte_flow_action_queue mirror2queue_conf = {.index = mirror2queue};
	struct rte_flow_action_queue fwd2queue_conf = {.index = fwd2queue};
	struct rte_flow_action_mark mirror_mark = {.id = 0xbefeb};
